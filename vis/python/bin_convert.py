...
def read_binary_fc(filename):
    ...
    while fp.tell() < filesize:
        ...
        data = np.fromfile(
            fp,
            dtype=np.float64 if varfmt == "d" else np.float32,
            count= -1 #nx1_out * nx2_out * nx3_out * n_vars,
        )
    ...
    filedata["nx2_mb"] = nx2
    filedata["nx3_mb"] = nx3
    filedata["nx1_out_mb"] = (mb_index[0][1] - mb_index[0][0]) + 2
    filedata["nx2_out_mb"] = (mb_index[0][3] - mb_index[0][2]) + 2
    filedata["nx3_out_mb"] = (mb_index[0][5] - mb_index[0][4]) + 2
